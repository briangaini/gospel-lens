...
**Worth knowing (not a bug): the PWA service worker can serve a stale cached bundle for a while after a deploy.** Discovered directly while verifying the footer-links fix above — a browser tab that had visited before kept running an old JS bundle after a fresh deploy until the service worker + its caches were explicitly cleared. This is standard Workbox/PWA behavior (by design, for offline support), not a regression from anything in this session, and it self-resolves on a visitor's next natural visit once the new service worker finishes its background install/activate cycle. Mentioned here so a "why isn't my change showing" moment doesn't get mistaken for a broken deploy. See the idea below for a possible fix.

**Proposed "next level" ideas (not yet greenlit):**
- **New — an "Update available" prompt instead of silently waiting on the PWA cache.** Right now a returning visitor with the site already installed/cached can be a deploy or two behind until the service worker's own background cycle catches up (see "Worth knowing" above) — normal PWA behavior, but a small toast ("A new version of the site is available — Refresh") using `vite-plugin-pwa`'s update-prompt mode would let people jump to the latest version immediately instead of waiting on it. Small, low-risk addition to the existing PWA setup.
