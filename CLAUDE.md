...
**Follow-up (2026-09-04): sharing with a file attached was also passing a separate `url` field, and that caused real problems on desktop.** Brian reported two symptoms sharing from a Mac: a share showing "the old white logo" as a second, unrelated image alongside the actual card (with the verse as separate text underneath), and picking "Copy" from the native share sheet producing two images of the same card when pasted. Root cause: passing `url` as its own structural field *alongside* `files` makes some share destinations (and the OS share sheet's own "Copy" action) independently fetch a rich link preview for that URL — pulling in the site's own generic icon/og-image completely separately from the actual attached card, which explains both symptoms as the same cause (a second, unrelated image competing with the real one). Fixed by dropping the separate `url` field entirely whenever a file is being shared — nothing is lost, since the link is already folded into `text`. `url` is still passed in the text-only fallback branch, where there's no attached file for it to conflict with. Verified via mocked `navigator.share()` calls, live on production, that the files-branch call now sends exactly `{title, text, files}` with no `url` key; the multi-format clipboard fallback for browsers without native share was untouched (never used a separate `url` field to begin with) and re-confirmed still correct. **Honest limit:** the real macOS/Windows native share panel itself can't be driven from here to fully replicate Brian's exact repro — verification stopped at confirming the API call itself is now correct; Brian still needs to re-test on his own machine to confirm the actual fix before this line gets marked resolved with confidence.
